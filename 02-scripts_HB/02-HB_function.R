...
#1-HB function ####
  HB<-function(param, input_rcn, input_rcn_gauging, input_climate, simul_period, nb_core, observed_flow_month, gauging){
    gc()
      ...
      #1.5.1-Parallel loop ####
        ...
        climate_data_VI<-foreach(k = 1 : (length(unique(climate_data$climate_cell))), .combine = rbind, .inorder = FALSE) %dopar% {
          ...
          #1.5.1.3-compute Oudin PET ####
            input_dd$PET<-round(PE_Oudin(JD = yday(as.POSIXct(paste(input_dd$year, input_dd$month, input_dd$day, sep = "-"), format="%Y-%m-%d")),
                                         ...
                                         LatUnit = "deg", TimeStepIn = "daily", TimeStepOut = "daily"), 2)
            rm(y)
            input_dd
    ...
    #1.6-run HB water budget partitioning ####
      #1.6.1-Model loop by grid cell in parallel #####
        ...
        water_budget<-foreach(j = 1 : (length(unique_rcn_cell)), .combine = rbind, .inorder = FALSE) %dopar% {
          ...
          #1.6.1.4-RCN variations based on API ####
            rcn_api <- data.table("year" = as.numeric(rcn_climate$year),
            ...
            for (ii in 1:nrow(rcn_climate)){
              if(ii == 1){
                ...
                gwr[ii]<-ifelse(rcn_subset$RCNII == 100,                                            #is the cell water or wetland?
                                0,                                                                  #if yes, no gwr
                                (budget1[ii]-aet[ii])*((budget1[ii]/sw_m)*f_inf))                   #if not, normal gwr computation
          ...
          #1.6.1.8- compute results ####
            w_b <- data.table("climate_cell" = as.integer(rcn_climate$climate_cell),
      ...
      #1.6.2-write results ####
        fwrite(water_budget, paste("01_bilan_spat_month.csv", sep = ""))
        budget_unspat <-water_budget[,.(VI = mean(VI),
                                        t_mean = mean(t_mean),
                                        runoff = mean(runoff),
                                        pet = mean(pet),
                                        aet = mean(aet),
        ...
        fwrite(budget_unspat, paste("02_bilan_unspat_month.csv", sep = ""))
    ...
    #1.7-result by gauging station, error computation ####
      for (st in 1:length(gauging)){
        #1.7.1-load the modeled data per gauging station ####
            ...
            budget_month<- budget_month[,.(VI = mean(VI),
                                           t_mean = mean(t_mean),
                                           runoff = mean(runoff),
                                           pet = mean(pet),
                                           aet = mean(aet),
        ...
        #1.7.2-Create the comparison data frame ####
            cols<-which(colnames(observed_flow_month) %in% c("year", "month", as.character(unique(budget_month$gauging_stat)), 
        ...
        #1.7.3-save the simulation results by gauging station ####
            fwrite(comparison_month, paste("03_bilan_unspat_month_", gauging[st], ".csv", sep = ""))
        ...
        #1.7.4-Objective functions ####
            ...
            error_ind[1,1]<-KGE(m_q_month_cal[which(!is.na(o_q_month_cal))],
                                o_q_month_cal[which(!is.na(o_q_month_cal))], na.rm = TRUE)
            error_ind[1,2]<-KGE(m_q_month_val[which(!is.na(o_q_month_val))],
                                o_q_month_val[which(!is.na(o_q_month_val))], na.rm = TRUE)
            ...
            #Clean 
            rm(m_q_month_cal, o_q_month_cal, m_q_month_val, o_q_month_val, m_baseflow_month_cal, o_baseflow_month_cal,
               m_baseflow_month_val, o_baseflow_month_val)
            
        #1.7.5-write the simulation metadata in a datatable and save it ####
            if(st == 1){
              simulation_metadata<-data.table(gauging_stat = unique(budget_month$gauging_stat),
                                              ...
                                              qtot_sim = mean(budget_month[,.(qtot = sum(runoff+runoff_2 +gwr, na.rm = TRUE)),
                                                                           .(year)][[2]]),
                                              aet_sim = mean(budget_month[,.(aet = sum(aet, na.rm = TRUE)),
                                                                          .(year)][[2]]),
                                              gwr_sim = mean(budget_month[,.(gwr = sum(gwr, na.rm = TRUE)),
                                                                          .(year)][[2]]),
              ...
              simulation_metadata<-rbind(simulation_metadata, 
                                         data.table(gauging_stat = unique(budget_month$gauging_stat),
                                                    cal_beg = calibration_start, cal_end = calibration_end, val_beg = validation_start, val_end = validation_end,
                                                    T_snow= T_snow, T_m= T_m, C_m = C_m, TT_F = TT_F, F_T = F_T, t_API = t_API,
                                                    f_runoff = f_runoff, sw_m = sw_m, f_inf = f_inf,
                                                    KGE_qtot_cal = error_ind[1,1], KGE_qbase_cal = error_ind[2,1],
            ...
            if(st == length(gauging)){
              simulation_metadata[nrow(simulation_metadata), c("KGE_mean_cal", "KGE_mean_val") := as.list(c(mean((simulation_metadata$KGE_qtot_cal+simulation_metadata$KGE_qbase_cal)/2), 
                                                                                                            ...
                                                                                                            na.rm = TRUE))[1:2]]
              fwrite(simulation_metadata, "04-simulation_metadata.csv")
    ...
    #1.8-export raster for interannual runoff, aet and GWR ####
      budget_month_spat<-water_budget[,.(runoff = sum(runoff + runoff_2, na.rm = TRUE), aet = sum(aet, na.rm = TRUE), gwr = sum(gwr, na.rm = TRUE)),
                                      .(rcn_cell, year)]
      ...
      writeRaster(runoff, filename = "05_interannual_runoff_NAD83.tif",Format = "GTiff", bylayer = TRUE, overwrite = TRUE)
      ...
      aet<-setMinMax(aet)
      writeRaster(aet, filename = "06_interannual_aet_NAD83.tif", Format = "GTiff", bylayer = TRUE, overwrite = TRUE)
      ...
      gwr<-setMinMax(gwr)
      writeRaster(gwr, filename = "07_interannual_gwr_NAD83.tif", Format = "GTiff", bylayer = TRUE, overwrite = TRUE)
    
    #1.9-Clean ####
      rm(st, comparison_month, budget_unspat, observed_flow_month, input_rcn_gauging, water_budget, input_rcn, climate_data,
